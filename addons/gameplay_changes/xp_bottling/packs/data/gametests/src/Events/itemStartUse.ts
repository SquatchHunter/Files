...
world.afterEvents.itemStartUse.subscribe(({ source, itemStack }: ItemStartUseAfterEvent): void => {
	...
	// TODO: when itemStartUse fires, check if "instant_use" is enabled and skip setting prop or timeout and jump straight to action ✔️
	const { configVersion, instantUse, timeToUse, stackMultiplier }: XpBottlingSettings = getSettings();
	if (!configVersion) {
	...
	// TODO: when itemStartUse fires, set a DynProp on using player ✔️
	void updatePlayerSettings(source, { usingSince: startTick });

	// TODO: when itemStartUse fires, start a timeout/interval to check back after configured "use_time" in settings ✔️
