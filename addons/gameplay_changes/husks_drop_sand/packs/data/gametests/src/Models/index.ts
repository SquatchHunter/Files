/**
 * Models Module
 *
 * This module exports the definitions of the types, interfaces and enums of the addon
 * It centralizes all model-related exports for easier import and management across different parts of the application.
 *
 * For more examples, refer to
 * @see addons/gameplay_changes/graves/packs/data/gametests/src/Models
 */
export { HusksDropSandSettings, HusksDropSandSettingsDynamicProperties, HusksDropSandDefaults } from './dynamicProperties';
export { HusksDropSandForbiddenDeaths } from './forbiddenDeaths';
